          ...
          <div className="relative h-full w-full flex flex-col items-center justify-center">
            ...
            >
              {tagRows.map((row) => (
                ...
                >
                  {[...row, ...row, ...row]
                    .map((item, idx) => ({ ...item, copyIndex: Math.floor(idx / row.length) }))
                    .map((item) => (
                    <div
            ...
            >
              <div className="relative">
                <MagnifyingLens size={DEFAULT_CONFIG.lensSize} />
                <div className="absolute top-[6px] left-[6px] w-[60px] h-[60px] rounded-full bg-white/10 pointer-events-none" />
              </div>
            </motion.div>
          </div>

          <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-zinc-50 dark:from-zinc-900 to-transparent z-20"></div>
          <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-zinc-50 dark:from-zinc-900 to-transparent z-20"></div>
        </div>
